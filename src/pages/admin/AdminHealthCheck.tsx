...
const TESTS: { name: string; fn: () => Promise<unknown> }[] = [
  ...
  { name: "Supabase: support_tickets", fn: async () => { const r = await supabase.from("support_tickets").select("id").limit(1); if (r.error) throw r.error; } },
  { name: "Supabase: vip_requests", fn: async () => { const r = await supabase.from("vip_requests").select("id").limit(1); if (r.error) throw r.error; } },
  { name: "Supabase: admin_shifts", fn: async () => { const r = await supabase.from("admin_shifts").select("admin_username").limit(1); if (r.error) throw r.error; } },
  { name: "Supabase: admin_accounts", fn: async () => { const r = await supabase.from("admin_accounts").select("id").limit(1); if (r.error) throw r.error; } },
