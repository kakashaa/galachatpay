...
const AdminIncomePage: React.FC = () => {
  ...
  const typeIcon = (type: string) => type === "vip" ? "👑" : type === "salary" ? "💰" : "🎁";

  return (
    <AdminPageLayout title="إدارة المداخيل" accentColor="#3b82f6" onLogout={handleLogout}>
      <div className="max-w-2xl mx-auto p-4 space-y-4" dir="rtl">
        {loading ? (
          ...
          <>
            {/* Hero Card — Blue */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-gradient-to-br from-blue-500/15 to-blue-600/5 border border-blue-500/20 rounded-3xl p-6 space-y-3">
              <div className="flex items-center gap-2">
                <DollarSign className="w-6 h-6 text-blue-400" />
                <span className="text-xs text-muted-foreground">إجمالي الأرباح</span>
              </div>
              <p className="text-4xl font-bold font-mono text-blue-400">{stats.totalDiamonds.toLocaleString()}</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-gradient-to-br from-blue-500/5 to-transparent border border-blue-500/10 rounded-2xl p-4">
                <p className="text-[10px] text-muted-foreground mb-1">دخل اليوم</p>
                <p className="text-2xl font-bold font-mono text-blue-400">+{stats.todayIncome.toLocaleString()}</p>
                <p className="text-[10px] text-muted-foreground">الماس</p>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-gradient-to-br from-sky-500/5 to-transparent border border-sky-500/10 rounded-2xl p-4">
                ...
                <div className="w-full h-2 bg-white/5 rounded-full mt-2 overflow-hidden">
                  <div className="h-full bg-sky-500 rounded-full transition-all" style={{ width: `${stats.monthlyGoal}%` }} />
                </div>
              </motion.div>
            </div>
              ...
              {recentLogs.map((log, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="flex items-center gap-3 p-3 bg-white/[0.02] border border-white/[0.04] rounded-xl">
                  ...
                  <div className="text-left">
                    <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold ${log.status === "approved" ? "bg-emerald-500/20 text-emerald-400" : log.status === "pending" ? "bg-amber-500/20 text-amber-400" : "bg-rose-500/20 text-rose-400"}`}>
