...
const AdminGiftsPage: React.FC = () => {
  ...
  return (
    <AdminPageLayout title="إدارة الهدايا" onLogout={handleLogout}>
        ...
        <div className="flex gap-1 bg-white/[0.03] rounded-xl p-1 border border-white/[0.06]">
          {[
            ...
            <button key={t.key} onClick={() => setSubTab(t.key)}
              className={`flex-1 py-2.5 rounded-lg text-[11px] font-bold transition-colors flex items-center justify-center gap-1 ${subTab === t.key ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
              {t.icon}{t.label}
            </button>
          ))}
        </div>
            ...
            {subTab === "stars" && (
              <div className="space-y-3">
                {starGifts.length === 0 ? (
                  ...
                  <div key={gift.id} className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4">
              ...
              <div className="space-y-3">
                ...
                {showAddEntry && (
                  <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4 space-y-3">
                    ...
                    <input type="file" accept="video/mp4,.webp,.svga,video/webm" onChange={(e) => setEntryFile(e.target.files?.[0] || null)} className="w-full text-sm file:mr-2 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-sm file:bg-primary file:text-primary-foreground bg-white/[0.03] border border-white/[0.06] rounded-lg p-1" />
                    <div className="flex gap-2">
                      {[1, 2, 3].map((lvl) => (
                        <button key={lvl} onClick={() => setNewEntry({ ...newEntry, star_level: lvl })} className={`flex-1 py-2 rounded-lg border text-sm font-bold flex items-center justify-center gap-1 ${newEntry.star_level === lvl ? "border-primary bg-primary/10" : "border-white/10"}`}>
                          {renderStars(lvl)}
                        </button>
                      ))}
                    </div>
              ...
              <div className="space-y-3">
                ...
                {showAddFrame && (
                  <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4 space-y-3">
                    ...
                    <input type="file" accept=".svga,.webp,image/webp" onChange={(e) => setFrameFile(e.target.files?.[0] || null)} className="w-full text-sm file:mr-2 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-sm file:bg-primary file:text-primary-foreground bg-white/[0.03] border border-white/[0.06] rounded-lg p-1" />
                    <div className="flex gap-2">
                      {[1, 2, 3].map((lvl) => (
                        <button key={lvl} onClick={() => setNewFrame({ ...newFrame, star_level: lvl })} className={`flex-1 py-2 rounded-lg border text-sm font-bold flex items-center justify-center gap-1 ${newFrame.star_level === lvl ? "border-primary bg-primary/10" : "border-white/10"}`}>
                          {renderStars(lvl)}
                        </button>
                      ))}
                    </div>
            ...
            {subTab === "custom" && (
              <div className="space-y-3">
                {customGifts.length === 0 ? (
                  ...
                  <>
                    <p className="text-xs font-bold text-amber-400">طلبات معلقة ({customGifts.filter((g: any) => g.status === "pending").length})</p>
                    {customGifts.filter((g: any) => g.status === "pending").map((gift: any) => (
                      <div key={gift.id} className="bg-white/[0.03] border border-amber-500/20 rounded-2xl p-4 space-y-2">
                        <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          <Button size="sm" className="flex-1 bg-emerald-600 hover:bg-emerald-700" onClick={async () => {
