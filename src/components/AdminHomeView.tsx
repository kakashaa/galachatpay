  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
      ...
      <div className="w-full h-1.5 rounded-full bg-zinc-800 overflow-hidden">
        <motion.div className={`h-full rounded-full ${barColor}`} initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 1 }} />
      </div>
    </div>
  );
};
...
const AdminHomeView: React.FC<Props> = ({
  adminDisplayName, stats, badges, recentLogs, isOwner, isSuperAdmin,
}: any) => {
  useExternalAssets();
  ...
  const menuItems = [
    ...
    { faIcon: "shield-halved", label: "الحماية", route: "/admin/ban", color: "text-rose-500", hoverBorder: "hover:border-rose-500", badge: banBadge },
    { faIcon: "money-bill-wave", label: "الرواتب", route: "/admin/salary", color: "text-green-500", hoverBorder: "hover:border-green-500", badge: salaryBadge },
    { faIcon: "clipboard-list", label: "الطلبات", route: "/admin/gifts", color: "text-blue-500", hoverBorder: "hover:border-blue-500", badge: requestsBadge },
    ...
    { faIcon: "clock-rotate-left", label: "السجل", route: "/admin/log", color: "text-indigo-500", hoverBorder: "hover:border-indigo-500", badge: 0 },
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: '#09090b', fontFamily: "'Cairo', 'IBM Plex Sans Arabic', sans-serif" }} dir="rtl">
      <div className="max-w-[448px] mx-auto px-4 pt-4 space-y-6">
        ...
        <div className="relative">
          <input
            ...
            dir="ltr"
          />
          <button onClick={() => searchUser()} className="absolute left-4 top-1/2 -translate-y-1/2">
            {searching
              ? <Loader2 className="w-5 h-5 animate-spin text-zinc-500" />
              : <FA icon="magnifying-glass" className="text-zinc-500" />
            }
          </button>
        </div>
          ...
          {searchResult && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-4">
              ...
              <div className="flex flex-col items-center gap-2">
                <div className="relative">
                  <img src={searchResult.avatar || '/placeholder.svg'} className="w-20 h-20 rounded-full object-cover ring-2 ring-zinc-800" alt=""
                    onError={(e) => { (e.target as HTMLImageElement).src = '/placeholder.svg'; }} />
                  {searchResult.vip_level > 0 && (
                    <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 text-[9px] px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-400 font-bold border border-amber-500/20 whitespace-nowrap">VIP {searchResult.vip_level}</span>
                  )}
                </div>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 space-y-2">
                {[
                  ...
                  { l: "الحالة", v: searchResult.is_banned ? '🔴 محظور' : '🟢 نشط', c: searchResult.is_banned ? 'text-red-400' : 'text-emerald-400' },
                ].map(r => (
              ...
              <div className="grid grid-cols-3 gap-2">
                <button onClick={() => navigate(`/admin/vip?uuid=${searchResult.uuid}`)} className="bg-amber-500/[0.08] border border-amber-500/15 rounded-xl py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                ...
                <button onClick={() => {
                  if (searchResult.is_banned) {
                    fetch('https://galachat.site/project-z/api.php', { method: 'POST', body: new URLSearchParams({ action: 'admin_ban_user', admin_key: 'ghala2026owner', uuid: searchResult.uuid, unban: 'true', reason: 'admin_action', admin_name: adminUsername }) })
                      .then(r => r.json()).then(d => { if (d.success) { toast.success('تم فك الحظر'); searchUser(searchResult.uuid); } else toast.error('فشل فك الحظر'); }).catch(() => toast.error('خطأ'));
                  } else navigate(`/admin/ban?uuid=${searchResult.uuid}`);
                }} className={`${searchResult.is_banned ? 'bg-emerald-500/[0.08] border-emerald-500/15' : 'bg-rose-500/[0.08] border-rose-500/15'} border rounded-xl py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform`}>
                  {searchResult.is_banned ? <Unlock className="w-5 h-5 text-emerald-400" /> : <FA icon="shield-halved" className="text-lg text-rose-400" />}
                  <span className={`text-[9px] font-bold ${searchResult.is_banned ? 'text-emerald-400' : 'text-rose-400'}`}>{searchResult.is_banned ? 'فك الحظر' : 'حظر'}</span>
                </button>
                <button onClick={() => navigate(`/admin/id-change?uuid=${searchResult.uuid}`)} className="bg-violet-500/[0.08] border border-violet-500/15 rounded-xl py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                  <FA icon="id-card" className="text-lg text-violet-400" />
                  <span className="text-[9px] text-violet-400 font-bold">تغيير آيدي</span>
                </button>
                <button onClick={() => toast.info('جاري تصفير الراتب...')} className="bg-orange-500/[0.08] border border-orange-500/15 rounded-xl py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                ...
                <button onClick={() => toast.info('جاري إيقاف الشحن...')} className="bg-red-500/[0.08] border border-red-500/15 rounded-xl py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                  <FA icon="circle-xmark" className="text-lg text-red-400" />
                  <span className="text-[9px] text-red-400 font-bold">إيقاف الشحن</span>
                </button>
                <button onClick={() => toast.info('جاري فتح تغيير الصورة...')} className="bg-sky-500/[0.08] border border-sky-500/15 rounded-xl py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
        ...
        {!searchResult && (
          <>
            ...
            <div className="bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-2xl p-5">
              ...
              <button onClick={() => navigate('/admin/gifts')} className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-lg text-sm font-medium hover:bg-white/30 transition-colors">
                عرض الطلبات
              </button>
            </div>
            ...
            {recentLogs.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  {isOwner && (
                    <button onClick={() => navigate("/admin/log")} className="text-xs text-emerald-500 font-medium">عرض الكل</button>
                  )}
                </div>
                    ...
                    return (
                      <div key={log.id || i} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-zinc-500 truncate mt-0.5">{log.details ? JSON.stringify(log.details).slice(0, 60) : ''}</p>
